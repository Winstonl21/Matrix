...
int s21_eq_matrix(matrix_t *A, matrix_t *B) {
  int status = SUCCESS;
  if (s21_check_matrix(A) == SUCCESS && s21_check_matrix(B) == SUCCESS) {
...
double s21_calc_determinant(matrix_t *A) {
  ...
  if (A->rows == 1) {
    ...
    calc = (A->matrix[0][0] * A->matrix[1][1]) -
...
double s21_calc_minor(matrix_t *A, int row_skip, int col_skip) {
  ...
  matrix_t minor = {NULL, 0, 0};
  if (OK == s21_create_matrix(A->rows - 1, A->columns - 1, &minor)) {
    s21_fill_minor(A, minor, row_skip, col_skip);
...
int s21_inverse_matrix(matrix_t *A, matrix_t *result) {
  ...
  if (result) {
    ...
    if (status == OK && tmp != 0.0) {
      ...
      s21_transpose(&m1, &m2);
      s21_mult_number(&m2, 1.0 / tmp, result);
...
//   B.matrix[0][0] = 1.2535;
//   B.matrix[0][1] = 25.7548;
//   B.matrix[0][2] = 3.7359;
//   B.matrix[1][0] = 85.3567889;
//   B.matrix[1][1] = 124;
//   B.matrix[1][2] = 178.0;
//   B.matrix[2][0] = 1.0;
//   B.matrix[2][1] = 5.0;
